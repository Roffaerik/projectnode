    ...
    login(request, response) {
        ...
        mysql_1.pool.getConnection((err, connection) => {
            connection.query('SELECT * FROM users WHERE email = ?', [email], (error, results, fileds) => {
                connection.release();
                if (error) {
                    return response.status(400).json({ error: "Error na sua aplicação!" });
                }
                (0, bcrypt_1.compare)(password, results[0].password, (err, result) => {
                    if (err) {
                        return response.status(400).json({ error: "Error em sua aplicação!" });
                    }
                    if (result) {
                        ...
                        console.log(token);
                        return response.status(200).json({ token: token, mensage: 'Autenticado com sucesso' });
                    }
                });
            });
        });
